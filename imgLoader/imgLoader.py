...
from MaranusHanusCitrus.custom.operation import get_files_in_dir_by_key
import tqdm


class imgLoader():
    def __init__(self, resize:int, rd_crop:int, batch_size:int, dir:str, ext:str,
                    to_gray:bool=False,
                    blur:bool=False, prob_blur:float=0.1,
                    jpeg:bool=False, prob_jpeg:float=0.1,
                    flip:bool=False, prob_flip:float=0.2):
        """
    ...
    def get_files_in_dir(self, shuffle=True, num_file:int=0)->None:
        """
    ...
    def load_img_in_dir(self)->None:
        ...
        if self.batch_size < len(self.files_itr):
            ...
            for i in range(1, self.batch_size):
                ...
                self.img = T.cat((self.img, cur_img), 0)  # Concatenate the last image to the previous tensor
                del cur_img
            self.cur_files = self.files_itr[:self.batch_size]  # Save the paths of the images at the current step
    ...
    def get_rd_batch(self)->T.Tensor:
        """
        To create a random subset from the entire list of images.
        @Params:
            self;
        @Return:
            rd_batch: The random subset from the entity;
        """
        this_lst_files = self.files
        random.shuffle(this_lst_files)
        this_lst_files = this_lst_files[:self.batch_size]
        rd_batch = self.load_imgs_in_lst(this_lst_files)
        return rd_batch
    ...
    def next_epoch(self):
        """
        To proceed to the next epoch and reset the list of files.
        Params:
            self;
        Return:
        """
    ...
    def parse_azmt_list(self, azmt_list_pth:str)->dict:
        ...
        for i, line in enumerate(f):
            ...
            dict_ai.update({cur_line[0]:np.array(list(map(float, cur_line[1:])))})
        return dict_ai
    ...
    def rd_gaussian_blur(self, img:Any, prob:float=0.1)->Any:
        ...
        if prob > prob_:
            ...
            tf = transforms.Compose([transforms.GaussianBlur(kernel_size=kernel_size, sigma=(0.1, 3.))])
            img_blurred = tf(img)
            return img_blurred
        else:
            return img
